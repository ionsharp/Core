...
namespace Imagin.Core.Linq
{
    ...
    {
        public static TimeSpan Coerce(this TimeSpan input, TimeSpan maximum, TimeSpan minimum = default)
        {
        ...
        public static int Months(this TimeSpan input) => (input.Days.Double() / 30).Floor().Int32();
        ...
        async public static Task TrySleep(this TimeSpan input) => await Task.Run(() => Try.Invoke(() => System.Threading.Thread.Sleep(input.TotalMilliseconds.Int32()), e => Log.Write<TimeSpan>(e)));

        public static int Years(this TimeSpan input)
        {
            var result = input.Months();
            return result >= 12 ? (result.Double() / 12).Floor().Int32() : 0;
        }
    }
}