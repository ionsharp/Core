...
public static partial class XList
{
    ...
    public static bool ContainsKey(this IList input, int key) => key >= 0 && key < input.Count;
    ...
    public static IEnumerable<T> Where<T>(this IList input, Predicate<T> predicate = null)
    {
        foreach (var i in input)
        {
            if (i is T t)
