...
namespace Imagin.Common.Numbers
{
    ...
    public class DoubleMatrix : Matrix<double>
    {
        ...
        /// </summary>
        [Hidden]
        public ObservableCollection<double> WeightedValues
        {
            get
            {
                var result = new ObservableCollection<double>();
                
                var smallest = double.MaxValue; var largest = double.MinValue;
                Each((x, y, i) =>
                {
                    if (i > largest)
                        largest = i;
                ...
                var range = new DoubleRange(smallest, largest);
                Each((x, y, i) =>
                {
                    result.Add(range.Convert(0, 1, i));
                    return i;
                });
                return result;
            }
