...
namespace Imagin.Common.Input.HotKeys
{
    ...
    public class HotKeySet
    {
         ...
         */
        readonly Dictionary<Keys, Keys> m_remapping;       //Used for mapping multiple keys to a single key
        readonly Dictionary<Keys, bool> m_hotkeystate;     //Keeps track of the status of the set of Keys

        //These provide the actual status of whether a set is truly activated or not.
        int m_hotkeydowncount;   //number of hot keys down
        int m_remappingCount;    //the number of remappings, i.e., a set of mappings, not the individual count in m_remapping

        readonly IEnumerable<Keys> m_hotkeys;  //hotkeys provided by the user.
        bool m_enabled = true;       //enabled by default
        ...
        void InvokeHotKeyHandler( HotKeyHandler hotKeyDelegate )
        {
        ...
        Keys GetExclusiveOrPrimaryKey( Keys k )
        {
