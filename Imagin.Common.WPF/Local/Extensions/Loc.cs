using Imagin.Common.Local.Converters;
...
namespace Imagin.Common.Local.Extensions
{
    /// <summary>
    ...
    public class LocExtension : NestedMarkupExtension, INotifyPropertyChanged, IDictionaryEventListener, IDisposable
    {
        ...
        #region Variables
        private static readonly object ResourceBufferLock = new();
        private static readonly object ResolveLock = new();

        private static Dictionary<string, object> _resourceBuffer = new();
        ...
        public override object FormatOutput(TargetInfo endPoint, TargetInfo info)
        {
                    ...
                    if (name != cacheDPName)
                    {
                        ...
                        cacheDPKey = mi.Invoke(null, new object[] { name + ".Key", typeof(LocExtension) }) as DependencyProperty
                            ?? DependencyProperty.RegisterAttached(name + ".Key", typeof(string), typeof(LocExtension),
                                            new FrameworkPropertyMetadata("", (d, e) => { (d?.GetValue(cacheDPThis) as LocExtension)?.UpdateNewValue(); }));
                        cacheDPName = name;
                    }
            ...
            else if (endPoint.TargetObject is FrameworkContentElement element)
                epName = element.GetValueSync<string>(FrameworkContentElement.NameProperty);
            ...
            if (result == null)
            {
                ...
                else
                {
                        ...
                        && !string.IsNullOrEmpty(_key) && (targetType == typeof(string) || targetType == typeof(object)))
                    {
                        ...
                        {
                            result = Format?.F(_key) ?? _key;
                            result = LocalizeDictionary.MissingKeyFormat.F(Prefix, result, Suffix);
                        }
                    }
                }
            }
        ...
        public bool SetBinding(object targetObject, object targetProperty, int targetPropertyIndex)
        {
            ...
            if (targetProperty is DependencyProperty property)
                ...
                targetPropertyType = info.PropertyType;
