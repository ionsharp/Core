...
namespace Imagin.Common.Storage
{
    ...
    public class ShellContextMenu : NativeWindow
    {
        ...
        public static Result Show(Point point, params FileInfo[] items)
        {
            return Try.Invoke(() =>
            {
                var instance = new ShellContextMenu();
                instance.ReleaseAll();
                instance._arrPIDLs = instance.GetPIDLs(items);
                instance.Show(point);
            });
        }

        public static Result Show(Point point, params DirectoryInfo[] items)
        {
            return Try.Invoke(() =>
            {
                var instance = new ShellContextMenu();
        ...
        public static Result Show(Point point, params FileSystemInfo[] items)
        {
            return Try.Invoke(() =>
            {
                var instance = new ShellContextMenu();
                ...
                foreach (var i in items)
                {
                    if (i is FileInfo)
                    {
                        files.Add((FileInfo)i);
                    }
                    if (i is DirectoryInfo)
                    {
                        folders.Add((DirectoryInfo)i);
                    }
                }

                var _files = instance.GetPIDLs(files.ToArray()) ?? new IntPtr[0];
                var _folders = instance.GetPIDLs(folders.ToArray()) ?? new IntPtr[0];

                instance._arrPIDLs = new IntPtr[_files.Length + _folders.Length];

                for (var i = 0; i < _files.Length; i++)
                    instance._arrPIDLs[i] = _files[i];

                for (var i = _files.Length; i < _files.Length + _folders.Length; i++)
                    instance._arrPIDLs[i] = _folders[i - _files.Length];

                instance.Show(point);
            });
        }

        private void Show(Point point)
        {
            IntPtr pMenu = IntPtr.Zero,
                iContextMenuPtr = IntPtr.Zero,
            ...
            try
            {
                ...
                if (nSelected != 0)
                {
                    InvokeCommand(_oContextMenu, nSelected, _strParentFolder, point);
                }
            }
            catch (Exception e)
            {
    ...
    #region internal static class ShellHelper
