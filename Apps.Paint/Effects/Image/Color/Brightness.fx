...
{ 
    ...
    if (Target == 0)
    {
        float oldValue = hsb[2];

        //Yellow
        if (color.r > color.b && color.g > color.b)
        {
            hsb[2] += (oldValue * Yellow / 100);
        }
        //Magenta
        else if (color.r > color.g && color.b > color.g)
        {
            hsb[2] += (oldValue * Magenta / 100);
        }
        //Cyan
        else if (color.g > color.r && color.b > color.r)
        {
            hsb[2] += (oldValue * Cyan / 100);
        }

        //Red
        if (color.r > color.g && color.r > color.b)
        {
            hsb[2] += (oldValue * Red / 100);
        }
        //Green
        else if (color.g > color.r && color.g > color.b)
        {
            hsb[2] += (oldValue * Green / 100);
        }
        //Blue
        else if (color.b > color.r && color.b > color.g)
        {
            hsb[2] += (oldValue * Blue / 100);
        }
    }
    //[1] Threshold
    else if (Target == 1)
    {
        ...
        if ((threshold > 0 && oldValue > absoluteThreshold) || (threshold < 0 && oldValue < absoluteThreshold))
        {
            hsb[0] = hsb[0];
            hsb[1] = hsb[1];
            hsb[2] = oldValue + (oldValue * thresholdAmount);
        }
        else
        {
    ...
    else if (Target == 2)
    {
        ...
        {
            hsb[2] += hsb[2] * ShadowAmount;
        }
        //Midtone
        else if (hsb[2] <= MidtoneRange)
        {
            hsb[2] += hsb[2] * MidtoneAmount;
        }
        //Highlight
        else if (hsb[2] <= HighlightRange)
        {
            hsb[2] += hsb[2] * HighlightAmount;
        }
    }
