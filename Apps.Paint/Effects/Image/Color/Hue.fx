...
float ThresholdAmount : register(C14);
...
float4 main(float2 uv : TEXCOORD) : COLOR
{
    ...
    if (Target == 0)
    {
        float oldValue = hsb[0];

        //Yellow
        if (color.r > color.b && color.g > color.b)
        {
            hsb[0] += (oldValue * Yellow / 100);
        }
        //Magenta
        else if (color.r > color.g && color.b > color.g)
        {
            hsb[0] += (oldValue * Magenta / 100);
        }
        //Cyan
        else if (color.g > color.r && color.b > color.r)
        {
            hsb[0] += (oldValue * Cyan / 100);
        }

        //Red
        if (color.r > color.g && color.r > color.b)
        {
            hsb[0] += (oldValue * Red / 100);
        }
        //Green
        else if (color.g > color.r && color.g > color.b)
        {
            hsb[0] += (oldValue * Green / 100);
        }
        //Blue
        else if (color.b > color.r && color.b > color.g)
        {
            hsb[0] += (oldValue * Blue / 100);
        }
    }
    //[1] Threshold
    else if (Target == 1)
    {
        ...
        {
            hsb[0] = oldValue + (oldValue * thresholdAmount);
            hsb[1] = hsb[1];
            hsb[2] = hsb[2];
        }
        else
    ...
    else if (Target == 2)
    {
        ...
        {
            hsb[0] += hsb[0] * ShadowAmount;
        }
        //Midtone
        else if (hsb[2] <= MidtoneRange)
        {
            hsb[0] += hsb[0] * MidtoneAmount;
        }
        //Highlight
        else if (hsb[2] <= HighlightRange)
        {
            hsb[0] += hsb[0] * HighlightAmount;
        }
    }
