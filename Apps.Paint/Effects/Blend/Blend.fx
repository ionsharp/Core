sampler2D InputA : register(s0);
sampler2D InputB : register(s1);
sampler2D InputC : register(s2);
...
#define BlendNegation(base, blend)	clamp(float3(1.0, 1.0, 1.0) - abs(clamp(float3(1.0, 1.0, 1.0) - base - blend, 0, 1)), 0, 1)
...
float4 Blend(float2 xy, float4 inputColor, float4 blendColor)
{
...
{ 
	float4 color1 = tex2D(InputA, xy);
	float4 color2 = tex2D(InputB, xy);

	if (Mask > 0)
	{
		float4 color3 = tex2D(InputC, xy);
