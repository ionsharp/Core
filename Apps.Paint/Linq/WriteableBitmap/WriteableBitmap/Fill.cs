...
namespace System.Windows.Media.Imaging
{
    ...
    public unsafe static partial class WriteableBitmapExtensions
    {
        ...
        {
            var color1 = Convert(target);
            var color2 = Convert((sa << 24) | (sr << 16) | (sg << 8) | sb);
            var color3 = color1.Blend(color2, mode ?? BlendModes.Normal);
            return ConvertColor(color3);
        }
        ...
        public static void Blend(this WriteableBitmap a, WriteableBitmap b, BlendModes blendMode = BlendModes.Normal)
        {
            a.ForEach((x, y, color) =>
            {
                if (x < b.PixelWidth && y < b.PixelHeight)
                    return color.Blend(b.GetPixel(x, y), blendMode, 1);
        ...
        {
            var colors = new Matrix<Color>(b.Rows, b.Columns);
            b.Each((y, x, i) =>
            {
                colors.SetValue(y.UInt32(), x.UInt32(), color.A((i / 255 * opacity * 255).Byte()));
                return i;
            });
            ...
            a.ForEach((x, y, color) =>
            {
                if (everything || color.A > 0)
                {
                    ...
                    if (x + xOffset < b.Columns && y + yOffset < b.Rows)
                        return color.Blend(b[(uint)(y + yOffset), (uint)(x + xOffset)], blendMode, opacity);
                }
                return color;
            });
        }
            ...
            a.ForEach((x, y, color) =>
            {
                if (everything || color.A > 0)
                {
                    ...
                    if (x + xOffset < b.PixelWidth && y + yOffset < b.PixelHeight)
                        return color.Blend(b.GetPixel(x + xOffset, y + yOffset), blendMode, opacity);
                }
                return color;
            });
        }
        ...
        public static void Blend(this WriteableBitmap a, BlendModes blendMode, WriteableBitmap b, Point point, double opacity = 1)
        {
            a.ForEach((x0, y0, color) =>
            {
                if (x0 >= point.X && y0 >= point.Y)
                {
                    ...
                    var x2 = x1.Coerce(b.PixelWidth).Int32();
                    var y2 = y1.Coerce(b.PixelHeight).Int32();

                    if (x1 < b.PixelWidth && y1 < b.PixelHeight)
                        return color.Blend(b.GetPixel(x2, y2), blendMode, opacity);
                }
                return color;
            });
        }
        ...
        public static void BlendAt(this WriteableBitmap input, BlendModes blendMode, System.Drawing.Point point, System.Drawing.Color fill, double opacity, byte tolerance)
        {
            bool Similar(Color a, Color b)
            {
                ...
                var ab = a.Int32().GetBrightness();
                var bb = b.Int32().GetBrightness();

                var cb = (ab - bb).Absolute() * 255f;
                return cb <= tolerance;
            }

            var old = input.GetPixel(point.X, point.Y).Int32();

            var height = input.PixelHeight.UInt32();
            var width = input.PixelWidth.UInt32();
            ...
            if (input.GetPixel(point.X, point.Y) != old.Double())
                return;

            Queue<System.Drawing.Point> queue = new();
            input.SetPixel(point.X, point.Y, fill.Double());
            queue.Enqueue(point);
            ...
            while (queue.Count > 0)
            {
                ...
                System.Windows.Media.Color c = default; System.Drawing.Point p = default;

                //Left
                p = new System.Drawing.Point(next.X - 1, next.Y);

                c = input.GetPixel(p.X, p.Y);
                if (p.X >= 0 && Similar(c, old.Double()))
                {
                    input.SetPixel(p.X, p.Y, c.Blend(fill.Double(), blendMode, opacity));
                    queue.Enqueue(p);
                }
                ...
                c = input.GetPixel(p.X, p.Y);
                if (p.X < width && Similar(c, old.Double()))
                {
                    input.SetPixel(p.X, p.Y, c.Blend(fill.Double(), blendMode, opacity));
                    queue.Enqueue(p);
                }
                ...
                c = input.GetPixel(p.X, p.Y);
                if (p.Y < height && Similar(c, old.Double()))
                {
                    input.SetPixel(p.X, p.Y, c.Blend(fill.Double(), blendMode, opacity));
                    queue.Enqueue(p);
                }
        ...
        {
            var result = b.Transform(i => Colors.Black.A(i));
            a.ForEach((x0, y0, color) =>
            {
                if (x0 >= point.X && y0 >= point.Y)
                {
                    ...
                    var x2 = x1.Coerce((int)b.Columns).Int32();
                    var y2 = y1.Coerce((int)b.Rows).Int32();

                    if (x1 < b.Columns && y1 < b.Rows)
                        return color.A(a => ((a / 255) - (b[y2, x2] / 255)).Coerce(1).Multiply(255).Byte());
                }
                return color;
            });
        }
        ...
        {
            var x2 = x1 + colors.Columns.Int32();
            var y2 = y1 + colors.Rows.Int32();
            ...
            using (var context = bmp.GetBitmapContext())
            {
                ...
                {
                    var j = ConvertColor(colors[0, wxy]);
                    sa = ((j >> 24) & 0xff);
                    sr = ((j >> 16) & 0xff);
                    sg = ((j >> 8) & 0xff);
                    sb = ((j) & 0xff);
                ...
                for (var y = startYPlusX1 + w; y < offset2; y += w, jkd++)
                {
                    ...
                    for (int i = 0; i < len; i++)
                    {
                        ...
                        var j = ConvertColor(colors[jkd, i.UInt32()]);
                        sa = ((j >> 24) & 0xff);
                        sr = ((j >> 16) & 0xff);
                        sg = ((j >> 8) & 0xff);
                        sb = ((j) & 0xff);
